...
ConfigurationMixin.__doc__ = dedent('''\
    Like `chicken_turtle_util.cli.ConfigurationMixin`, but `context.configuration` is of type `deep_genome.core.Configuration`
    
    Configuration is loaded from `core.conf` files using ConfigurationLoader.
    ''')

#
DataDirectoryMixin = cli.DataDirectoryMixin('deep_genome')
CacheDirectoryMixin = cli.CacheDirectoryMixin('deep_genome')
