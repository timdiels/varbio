...
class TestPipelineCLI(object):
    ...
    @pytest.mark.asyncio
    async def test_signal(self, temp_dir_cwd, signal):  # temp_dir_cwd as the subprocess puts cache in local directory
        '''
        When the pipeline controller receives SIGTERM or SIGINT, cancel task and exit non-zero.
        '''
        process = await asyncio.create_subprocess_exec('python', __file__, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
if __name__ == '__main__':
    # Helper of TestPipelineCLI::test_signal
